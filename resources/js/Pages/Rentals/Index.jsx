...
import { KeyRound, PlusCircle, Users, AlertTriangle, DollarSign, Search, Eye, FileDown } from 'lucide-react';
...
export default function RentalsIndex({ rentals, clients, summary, filters }) {
    ...
    const [form, setForm] = useState({
        client_id: clients?.[0]?.id ?? '', description: '', address: '', monthly_fee: '',
    ...
    function handleSubmit(e) {
        e.preventDefault();
        const clientId = form.client_id || clients?.[0]?.id || '';
        if (!clientId) {
            alert('No hay clientes registrados para crear un contrato de alquiler.');
            return;
        }

        router.post(route('rentals.store'), { ...form, client_id: clientId }, {
            onSuccess: () => { setShowModal(false); setForm({ client_id: clients?.[0]?.id ?? '', description:'', address:'', monthly_fee:'', start_date:'', end_date:'', payment_day:1, notes:'' }); },
        });
    }
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-600">Contratos y cobros mensuales</p>
                    </div>
                </div>
            ...
            <form onSubmit={applyFilters} className="flex flex-wrap gap-3 items-center">
                <div className="relative">
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)}
                        ...
                        className="bg-white border border-gray-200 text-gray-900 rounded-lg pl-9 pr-3 py-2 text-sm w-64" />
                </div>
                <select value={status} onChange={e => setStatus(e.target.value)}
                    className="bg-white border border-gray-200 text-gray-900 rounded-lg px-3 py-2 text-sm">
                    <option value="">Todos los estados</option>
                    <option value="active">Activos</option>
                    <option value="suspended">Suspendidos</option>
                    <option value="ended">Finalizados</option>
                </select>
                <button type="submit" className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg text-sm transition-colors">
                    Filtrar
                </button>
            </form>
                        ...
                        <tbody className="divide-y divide-gray-700/30">
                            ...
                            {rentals.data.map(r => (
                                <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-gray-700">{r.start_date}</td>
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <div className="inline-flex items-center gap-2">
                                            ...
                                            <a
                                                href={route('rentals.receipt.pdf', r.id)}
                    ...
                    <div className="flex justify-center gap-2 px-4 py-3 border-t border-gray-200">
                        {rentals.links.map((link, i) => (
                            <Link key={i} href={link.url ?? '#'}
                                className={`px-3 py-1 rounded text-sm ${link.active ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:text-gray-900'}`}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                    <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-lg shadow-2xl">
                        <div className="flex items-center justify-between p-5 border-b border-gray-200">
                            ...
                            <button onClick={() => setShowModal(false)} className="text-gray-600 hover:text-gray-900">✕</button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-xs text-gray-600 mb-1">Descripción / Bien alquilado *</label>
                                ...
                                <div>
                                    <label className="block text-xs text-gray-600 mb-1">Día de vencimiento (1–28) *</label>
                                ...
                                <div>
                                    <label className="block text-xs text-gray-600 mb-1">Fecha fin (opcional)</label>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => setShowModal(false)}
                                    className="px-4 py-2 text-gray-600 hover:text-gray-900 transition-colors text-sm">Cancelar</button>
                                <button type="submit"
