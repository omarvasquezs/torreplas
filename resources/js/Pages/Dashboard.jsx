    ...
    return (
        <DashboardLayout>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {stats.map(s => (
                    <div key={s.name}
                        className={`bg-gray-800/50 border rounded-xl p-5 flex items-center justify-between ${s.bg}`}>
                        <div>
                            <p className="text-gray-400 text-xs">{s.name}</p>
                            <p className={`text-2xl font-bold mt-1 ${s.color}`}>{s.value}</p>
                        </div>
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.color} bg-gray-700/50`}>
                            <s.icon size={20} />
                        </div>
                    </div>
                ))}
                            ...
                            {(() => {
                                ...
                                return salesChart.map(day => (
                                    <div key={day.date} className="flex items-center gap-3 text-sm">
                                        <span className="text-gray-400 w-20 shrink-0 text-xs">{day.date}</span>
                                        ...
                                        <span className="text-gray-300 font-mono text-xs w-20 text-right">S/ {fmt(day.total)}</span>
                                    </div>
                                ));
                            })()}
                        </div>
                        ...
                        <div className="space-y-3">
                            {topProducts.map((p, i) => (
                                <div key={p.product_id ?? i} className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className="text-gray-400 text-xs">{p.total_sold} uds</span>
                        ...
                        <div className="space-y-2">
                            {recentMovements.map(m => (
                                <div key={m.id} className="flex items-center gap-3 text-sm">
                                    ...
                                    <span className="flex-1 text-gray-300 truncate">{m.product?.name ?? '—'}</span>
                                    <span className={`font-bold text-xs ${m.type === 'entrada' ? 'text-green-400' : m.type === 'salida' ? 'text-red-400' : 'text-gray-300'}`}>
                    ...
                    {paymentSummary ? (
                        <div className="space-y-3">
                            <div className="flex justify-between items-center py-2 border-b border-gray-700/50">
                                <span className="text-gray-400 text-sm">Cuentas x Cobrar</span>
                                <span className="font-mono text-yellow-400 font-bold">S/ {fmt(paymentSummary.receivable)}</span>
                            </div>
                            <div className="flex justify-between items-center py-2 border-b border-gray-700/50">
                                <span className="text-gray-400 text-sm">Cuentas x Pagar</span>
