                ...
                <form onSubmit={submit} className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {[['IN', 'Ingreso', 'green'], ['OUT', 'Egreso', 'red']].map(([v, l, c]) => (
                                <button key={v} type="button" onClick={() => setData('type', v)}
                                    className={`py-2 rounded-lg text-sm font-medium transition ${data.type === v ? `bg-${c}-600 text-white` : 'bg-gray-100 text-gray-600 hover:bg-gray-600'}`}>
                                    {l}
                                </button>
                            ))}
                        </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" value={data.amount} onChange={e => setData('amount', e.target.value)}
                            className="w-full px-3 py-2 bg-gray-100 border border-gray-200 rounded-lg text-white text-sm focus:outline-none" placeholder="0.00" />
                        {errors.amount && <p className="text-red-400 text-xs mt-1">{errors.amount}</p>}
                    </div>
                    <div>
                        <label className="block text-sm text-gray-600 mb-1">Descripción</label>
                        <input type="text" value={data.description} onChange={e => setData('description', e.target.value)}
                            className="w-full px-3 py-2 bg-gray-100 border border-gray-200 rounded-lg text-white text-sm focus:outline-none" placeholder="Concepto del movimiento" />
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <Link href={route('cash.index')} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600 transition">
                            <ArrowLeft size={18} />
                        </Link>
                        <div>
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-gray-700/50">
                            ...
                            {movements.data.map(m => (
                                <tr key={m.id} className="hover:bg-gray-100/30">
                                    ...
                                    <td className="px-4 py-3 text-gray-600 text-xs">{m.created_at}</td>
