        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 w-96 space-y-4">
                ...
                <form onSubmit={submit} className="space-y-4">
                    <div>
                        <label className="block text-sm text-gray-400 mb-1">Saldo inicial (S/)</label>
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-400 text-sm mt-1">Control de efectivo y cuentas bancarias</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {cashRegisters.map(cr => (
                            <div key={cr.id} className="bg-gray-800/50 rounded-xl border border-gray-700/50 p-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    <p className="text-white font-medium">{cr.name}</p>
                                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${cr.is_open ? 'bg-green-500/20 text-green-400' : 'bg-gray-600/50 text-gray-400'}`}>
                        ...
                        {bankAccounts.length === 0 ? (
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-700/50">
                                    {bankAccounts.map(ba => (
                                        <tr key={ba.id} className="hover:bg-gray-700/30">
                                            <td className="px-4 py-3 text-white font-medium">{ba.bank_name}</td>
                                            <td className="px-4 py-3 font-mono text-gray-300 text-xs">{ba.account_number}</td>
                                            <td className="px-4 py-3 text-gray-400 capitalize">{ba.account_type}</td>
                                            <td className="px-4 py-3 text-gray-400">{ba.currency}</td>
