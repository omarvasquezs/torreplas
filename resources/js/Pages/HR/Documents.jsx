            ...
            <div className="space-y-6">
                ...
                <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-5">
                    ...
                    <form onSubmit={submit} encType="multipart/form-data" className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="block text-gray-400 text-xs mb-1">Nombre del documento</label>
                        ...
                        <div>
                            <label className="block text-gray-400 text-xs mb-1">Archivo</label>
                            <input ref={fileRef} type="file" onChange={e => setData('file', e.target.files[0])}
                                className="w-full bg-gray-700/50 border border-gray-600 text-white rounded-lg px-3 py-1.5 text-sm
                            ...
                            <tbody>
                                {documents.map(doc => (
                                    <tr key={doc.id} className="border-b border-gray-700/30 hover:bg-gray-700/20">
                                        ...
                                        <td className="p-3">
                                            <span className={`px-2 py-0.5 rounded-full text-xs ${TYPE_COLORS[doc.type] ?? 'bg-gray-500/20 text-gray-400'}`}>
                                                {doc.type}
                                            </span>
                                        </td>
                                        <td className="p-3 text-gray-400 text-xs font-mono truncate max-w-36">{doc.original_name}</td>
                                        <td className="p-3 text-gray-400 text-xs">{doc.created_at?.split('T')[0]}</td>
