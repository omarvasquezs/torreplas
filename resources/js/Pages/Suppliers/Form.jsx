        ...
        <DashboardLayout>
            ...
            <div className="max-w-2xl space-y-6">
                <div className="flex items-center gap-3">
                    <Link href={route('suppliers.index')} className="p-2 rounded-lg hover:bg-gray-700 text-gray-400 transition">
                        <ArrowLeft size={18} />
                    </Link>
                    <div>
                        <h1 className="text-2xl font-bold text-white">{isEdit ? 'Editar Proveedor' : 'Nuevo Proveedor'}</h1>
                        <p className="text-gray-400 text-sm mt-0.5">{isEdit ? `Editando: ${supplier.name}` : 'Registrar nuevo proveedor'}</p>
                    </div>
                </div>

                <form onSubmit={submit} className="space-y-6">
                    {/* Información Fiscal */}
                    <section className="bg-gray-800/50 rounded-xl border border-gray-700/50 p-5 space-y-4">
                        <h2 className="text-white font-semibold">Información Fiscal</h2>
                        <div className="grid grid-cols-3 gap-4">
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">Tipo Doc.</label>
                                <select
                    ...
                    <div className="flex gap-3">
                        ...
                        <Link href={route('suppliers.index')} className="px-5 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg text-sm transition">
                            Cancelar
                        </Link>
                    </div>
                </form>
